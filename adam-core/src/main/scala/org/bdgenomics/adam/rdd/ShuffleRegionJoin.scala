...
private trait SortedIntervalPartitionJoinWithVictims[T, U, RT, RU]
   ...
   * PERFORMANCE NOTES:
   *
   * At one point these were all variables and we built new collections and
   * reassigned the pointers every time. We fixed this by using trimStart() and
   * ++=() to improve performance. Overall, we see roughly 25% improvement
